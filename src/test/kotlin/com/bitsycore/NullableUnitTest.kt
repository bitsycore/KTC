...
class NullableUnitTest : TranspilerTestBase() {
    ...
    @Test fun nullableIntWithValue() {
        ...
        r.sourceContains("ktc_Int\$Optional x = (ktc_Int\$Optional){ktc_SOME, 42};")
    }
    ...
    @Test fun nullableStringWithValue() {
        ...
        r.sourceContains("ktc_String\$Optional s = (ktc_String\$Optional){ktc_SOME, ktc_core_str(\"hello\")};")
    }

    @Test fun nullableStringNull() {
        val r = transpileMain("var s: String? = null")
        r.sourceContains("ktc_String\$Optional s = (ktc_String\$Optional){ktc_NONE};")
    }
    ...
    @Test fun nullableReturnSignature() {
        ...
        """)
        // Return type is ktc_Int\$Optional
        r.sourceContains("ktc_Int\$Optional test_Main_findValue(ktc_Bool flag)")
    }
    ...
    @Test fun nullableReturnNull() {
        ...
        """)
        r.sourceContains("return (ktc_Int\$Optional){ktc_NONE};")
    }
    ...
    @Test fun nullableReturnValue() {
        val r = transpile("""
            ...
            fun main(args: Array<String>) { val a = findValue() }
        """)
        r.sourceContains("return (ktc_Int\$Optional){ktc_SOME, 42};")
    }
    ...
    @Test fun nullableReturnCallSite() {
        ...
        r.sourceContains("ktc_Int\$Optional a = test_Main_findValue();")
    ...
    @Test fun assignNullToNullableVar() {
        val r = transpileMain("var x: Int? = 42\nx = null")
        r.sourceContains("x = (ktc_Int\$Optional){ktc_NONE};")
    }

    @Test fun assignValueToNullableVar() {
        val r = transpileMain("var x: Int? = null\nx = 10")
        r.sourceContains("x = (ktc_Int\$Optional){ktc_SOME, 10};")
    }
    ...
    @Test fun passNullToNullableParam() {
        ...
        """)
        // Passing null literal → ktc_Int\$Optional{ktc_NONE}
        r.sourceContains("test_Main_show((ktc_Int\$Optional){ktc_NONE})")
    }
    ...
    @Test fun passValueToNullableParam() {
        ...
        """)
        // Passing non-null literal → ktc_Int\$Optional{ktc_SOME, 99}
        r.sourceContains("test_Main_show((ktc_Int\$Optional){ktc_SOME, 99})")
    }
